    ...
    OpenIDAuthorizationCodeInterface
{
    ...
    public function __construct($connection, $config = array())
    {
        ...
        $this->config = array_merge(array(
            ...
            // Added for LanGuide:
            'difficulty_levels_table' => 'difficulty_levels',
            'test_results_table' => 'test_results',
            'test_type_table' => 'test_type'
        ), $config);
    }
    ...
    protected function checkPassword($user, $password)
    {
        ...
        // Modified to use bcrypt for LanGuide
     ...
     * @return bool
     */
    public function setUser($username, $password, $scope, $firstName = null, $lastName = null)
    {
        ...
        if ($this->getUser($username)) {
            $stmt = $this->db->prepare($sql = sprintf('UPDATE %s SET password=:password, first_name=:firstName, last_name=:lastName, scope=:scope where username=:username', $this->config['user_table']));
        } else {
     ...
     * @return bool
     */
    public function testTypeExists($type)
    {
        $stmt = $this->db->prepare('SELECT * FROM test_type WHERE name=:type');
        $stmt->execute(compact('type'));
     ...
     * @return bool
     */
    public function difficultyExists($name)
    {
        $stmt = $this->db->prepare('SELECT * FROM difficulty_levels WHERE name=:name');
     ...
     * @return bool
     */
    public function addTestResult($username, $test_type, $difficulty, $accuracy)
    {
        $stmt = $this->db->prepare($sql = "INSERT INTO test_results (username, type, difficulty, accuracy) VALUES (:username, :test_type, :difficulty, :accuracy)");

        return $stmt->execute(compact('username', 'test_type', 'difficulty', 'accuracy'));
    }
    ...
    {
        // This string has been extended with LanGuide User API functionality
        ...
        CREATE TABLE {$this->config['client_table']} (
          ...
          PRIMARY KEY (client_id)
            ...
            CREATE TABLE {$this->config['difficulty_levels_table']} (
              name VARCHAR(32) NOT NULL,
              PRIMARY KEY (name)
            );

            INSERT INTO {$this->config['difficulty_levels_table']} (name) VALUES ('easy'), ('medium'), ('hard');

            CREATE TABLE {$this->config['test_type_table']} (
              name VARCHAR(32) NOT NULL,
              PRIMARY KEY (name)
            );

            INSERT INTO {$this->config['test_type_table']} (name) VALUES ('listening'), ('reading'), ('speaking'), ('vocabulary'), ('writing');

            CREATE TABLE {$this->config['test_results_table']} (
